          ...
          <div class="flex justify-between items-start">
            <div class="flex-1">
              ...
              <div class="space-y-1 text-xs text-gray-500">
                <p>\u{1F4C5} \${item.date || ''}</p>
                \${item.category ? \`<p>\u{1F3F7}\uFE0F \${item.category}</p>\` : ''}
                \${item.author ? \`<p>\u270D\uFE0F \${item.author}</p>\` : ''}
                \${item.source ? \`<p>\u{1F4F0} \${item.source}</p>\` : ''}
                \${item.tags && item.tags.length ? \`<p>\u{1F516} \${Array.isArray(item.tags) ? item.tags.join(', ') : item.tags}</p>\` : ''}
              </div>
            </div>
        ...
        newsForm.addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              alert('Failed to save news item: ' + (errorData.error || 'Unknown error'));
            }
          } catch (error) {
...
const $$file = "E:/Kalyanamitta/Kalyanamitta/kalyanamitta-web/src/pages/admin/news.astro";
